...
export function useDifyAPI() {
  ...
  const fetchConversationMessages = async (conversationId: string, userId: string) => {
    setIsLoading(true);
    try {
      const url = `${DIFY_CONFIG.BASE_URL}/messages?conversation_id=${conversationId}&user=${userId}`;
